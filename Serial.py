...
class SerialProtocolHandler:
    def __init__(self, port, baudrate=115200):
        ...
        self.data = {
            key: ([], False) for key in self.rx_map.keys()
        }  # 用于存储接收到的数据
    ...
    def receive_data(self):
        ...
        if self.rxstate == 0 and data == 0xAA:
            ...
            self.rx_data = []
            for values in self.rx_map[Data(self.rx_id)]:
                if values is float:
            ...
            if total_sum == ((self.check_high << 8) | self.check_low):
                ...
                self.check_low = 0
                self.check_high = 0
                self.data[Data(self.rx_id)] = (self.rx_data, True)
            else:
                self.rxstate = 0
                self.sum1 = 0
                self.data_len = 0
